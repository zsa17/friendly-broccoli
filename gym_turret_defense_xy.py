...
class TurretDefenseGym(gym.Env):
  ...
  def __init__(self):
    ...
    self.observation_space = spaces.Box(low=np.array([[-1,-1,0]]),
                                        high=np.array([[1, 1,1]]),
                                        dtype=np.float16)
  ...
  def step(self, action):
    ...
    self.state = [self.state[0] * self.state_scale[0],self.state[1] * self.state_scale[0], self.state[2] * self.state_scale[1]]

    if self.team == 0:
      #reward = self.c1 * .5 * (1 + np.cos(self.state[1])) + self.c2
      beta = np.arcsinh(self.state[1])/np.hypot(self.state[0],self.state[1])
      alpha = self.state[2] - beta

      reward = self.c1 * .5 * (1 + np.cos(alpha))
    elif self.team == 1:
      beta = np.arcsin(self.state[1])/np.hypot(self.state[0],self.state[1])
      alpha = self.state[2] - beta

      reward = -self.c1 * .5 * (1 + np.cos(alpha)) - self.c2
    ...
    if np.hypot(self.state[0],self.state[1])< self.terminal_state:
      terminated = True
  ...
  def reset(self,seed=None, options=None):
    ...
    self.time_steps = 0
    self.state = [random.uniform(.33, 1), random.uniform(.33, 1), random.uniform(0, 1)]
    #self.state = [50,0]
