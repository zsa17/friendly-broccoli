...
class TurretDefenseGym(gym.Env):
  ...
  def step(self, action):
    ...
    if self.team == 0:
      action = [self.passive_model.predict([self.state], deterministic=True)[0]*np.pi/2, action - 1]
      #action = [np.pi, action-1]

    elif self.team == 1:
      action = [action*np.pi/2, self.passive_model.predict([self.state], deterministic=True)[0] - 1]
    ...
    if self.team == 0 and self.time_steps > 1000:
      terminated = True
