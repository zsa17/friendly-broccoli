...
class TurretDefenseGymBase(gym.Env):
    ...
    """
        ...
        self.info = {"Test": "YOYO"}
        self.time_steps = 0
        self.terminal_state = 48
        # Initialize deque:
        self.state_send = collections.deque(np.zeros(self.observation_space.shape[1]))
    ...
    """
        ...
        if self.set_alpha:
            self.state = [random.uniform(self.terminal_state, self.state_scale[0]), self.set_alpha_value,
                          random.uniform(0, self.state_scale[1])]
        else:
            self.state = [random.uniform(self.terminal_state, self.state_scale[0]), random.uniform(0, self.state_scale[1]),
                          random.uniform(0, self.state_scale[1])]
